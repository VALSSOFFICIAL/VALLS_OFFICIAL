...
TIMES="10"
CHATID="-1002210291479"
KEY="6819742738:AAF47nFHt74q88XmQyK1OOASfnCNwGnqJD8"
...
clear;clear;clear
#echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "\033[96;1m          CHECK PERMISSIONS            \033[0m"
#echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
clear
sleep 3
echo -e "   successful permission "
sleep 5
...
echo -e "\e[32mloading...\e[0m"
clear
izinsc="https://raw.githubusercontent.com/VALSSOFFICIAL/regip/main/izin"
rm -f /usr/bin/user
...
if [[ -z "$nama" ]]; then
  echo "VALLS OFFICIAL" > /etc/xray/username
else
  echo "$nama" > /etc/xray/username
fi
echo ""
elif [[ $host == "2" ]]; then
wget ${REPO}Folder/cf.sh && chmod +x cf.sh && ./cf.sh
rm -f /root/cf.sh
clear
else
...
"'&reply_markup={"inline_keyboard":[[{"text":"ᴏʀᴅᴇʀ","url":"t.me/VALLS_OFFICIAL"},{"text":"Contack","url":"https://wa.me/6282299596662"}]]}'
...
wget -O /etc/systemd/system/runn.service "${REPO}Folder/runn.service" >/dev/null 2>&1
...
curl ${REPO}XRAY/nginx.conf > /etc/nginx/nginx.conf
...
wget -O /etc/pam.d/common-password "${REPO}Folder/password"
...
wget raw.githubusercontent.com/VALSSOFFICIAL/VALLS_OFFICIAL/main/Folder/limit.sh && chmod +x limit.sh && ./limit.sh
cd
wget -q -O /usr/bin/limit-ip "${REPO}Folder/limit-ip"
chmod +x /usr/bin/*
cd /usr/bin
sed -i 's/\r//' limit-ip
cd
...
wget -q -O /usr/local/kyt/udp-mini "${REPO}Folder/udp-mini"
chmod +x /usr/local/kyt/udp-mini
wget -q -O /etc/systemd/system/udp-mini-1.service "${REPO}Folder/udp-mini-1.service"
wget -q -O /etc/systemd/system/udp-mini-2.service "${REPO}Folder/udp-mini-2.service"
wget -q -O /etc/systemd/system/udp-mini-3.service "${REPO}Folder/udp-mini-3.service"
...
wget -q -O /tmp/nameserver "${REPO}Folder/nameserver" >/dev/null 2>&1
...
wget -q -O /etc/ssh/sshd_config "${REPO}Folder/sshd" >/dev/null 2>&1
...
wget -q -O /etc/default/dropbear "${REPO}XRAY/dropbear.conf"
...
wget ${REPO}Folder/openvpn &&  chmod +x openvpn && ./openvpn
...
printf "q\n" | rclone config
wget -O /root/.config/rclone/rclone.conf "${REPO}XRAY/rclone.conf"
cd /bin
...
wget -q -O /etc/ipserver "${REPO}Folder/ipserver" && bash /etc/ipserver
...
chronyc sourcestats -v
chronyc tracking -v
wget ${REPO}Folder/bbr.sh &&  chmod +x bbr.sh && ./bbr.sh
...
wget -O /etc/banner.txt "${REPO}banner.com"
print_success "Fail2ban"
}
function ins_epro(){
clear
print_install "Menginstall ePro WebSocket Proxy"
wget -O /usr/bin/ws "${REPO}Folder/ws" >/dev/null 2>&1
wget -O /usr/bin/tun.conf "${REPO}XRAY/tun.conf" >/dev/null 2>&1
wget -O /etc/systemd/system/ws.service "${REPO}Folder/ws.service" >/dev/null 2>&1
...
wget -O /usr/sbin/ftvpn "${REPO}Folder/ftvpn" >/dev/null 2>&1
